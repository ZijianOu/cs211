...
void popAndEval(MyStack &valStack, Operators &opStack){
  char op = opStack.top();
  opStack.pop();
  int v2 = valStack.top();
  //printf("%d \n", v2);
    ...
    /* The expression contains an OPERATOR */
    else if (inputToken.equalsType(OPERATOR))
      {
        ...
        if(inputToken.getOperator()=='+'||inputToken.getOperator()=='-'){
          while((opStack.isEmpty()==false)&&(opStack.top()=='+')||(opStack.top()=='-')||(opStack.top()=='*')||(opStack.top()=='/')||(opStack.top()=='%')){
            popAndEval(valStack, opStack);
          }
          opStack.push(inputToken.getOperator());
        }
        if((inputToken.getOperator()=='*')||(inputToken.getOperator()=='/')||(inputToken.getOperator()=='%')){
          while((opStack.isEmpty()==false)&&(opStack.top()=='*')||(opStack.top()=='/')||(opStack.top()=='%')){
            popAndEval(valStack, opStack);
          }
          opStack.push(inputToken.getOperator());
        }
        if(inputToken.getOperator()==')'){
          while((opStack.isEmpty()==false)&&(opStack.top()!='(')){
            popAndEval(valStack, opStack);
          }
          if(opStack.isEmpty()==true){
            ...
            tr->clearToEoln();
            valStack.setEmpty();
            delete[] valStack.getArr();
            delete[] opStack.getArr();
            return;
          }
          else{
            opStack.pop();
